...
const ReceiptVoucher = () => {
  ...
  const fetchReceipts = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_BASE_URL}/api/receipt/receipts`
    );
    if (data) {
  ...
  const getSingleReceipt = async (id) => {
    ...
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/receipt/receipt/${id}`
      );
      if (data) {
        setInputs(data.receipt);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/receipt/create`,
        {
          ...inputs,
        }
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/receipt/update/${rowId}`,
        {
          ...inputs,
        }
      );
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const { data } = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/receipt/delete/${rowId}`
      );
      if (data) {
                          ...
                          receipt?.map((item) => (
                            <>
                              <tr
                                title="Edit"
                                key={item._id}
                                onClick={() => getSingleReceipt(item._id)}
                                className={`${
                                  item._id == rowId ? "bg-cyan-400" : "bg-white"
                                } cursor-pointer`}
                              >
