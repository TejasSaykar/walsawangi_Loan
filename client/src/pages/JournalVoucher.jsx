...
const JournalVoucher = () => {
  ...
  const fetchJournals = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_BASE_URL}/api/journal/journals`
    );
    if (data) {
  ...
  const getSingleJournal = async (id) => {
    ...
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/journal/journal/${id}`
      );
      if (data) {
        setInputs(data.journal);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/journal/create`,
        {
          ...inputs,
        }
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/journal/update/${rowId}`,
        {
          ...inputs,
        }
      );
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const { data } = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/journal/delete/${rowId}`
      );
      if (data) {
                          ...
                          journal?.map((item) => (
                            <>
                              <tr
                                title="Edit"
                                key={item._id}
                                onClick={() => getSingleJournal(item._id)}
                                className={`${
