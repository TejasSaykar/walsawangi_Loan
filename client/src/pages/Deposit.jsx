...
const Deposit = () => {
  ...
  const search = (e) => {
    if (startDate && endDate) {
      const filtered = deposit.filter((item) => {
        ...
        return (
          depositDate >= new Date(startDate) && depositDate <= new Date(endDate)
        );
      });
      setDeposit(filtered);
  ...
  const fetchDeposits = async () => {
    try {
      const { data: deposits } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/deposit/deposits`
      );
      if (deposits) {
        console.log("Deposits : ", deposits.deposits);
        setDeposit(deposits.deposits);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/deposit/deposit`,
        {
          ...inputs,
        }
      );
  ...
  const fetchSingleDeposit = async (id) => {
    setCurrentId(id);
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/deposit/deposit/${id}`
      );
      if (data) {
        setInputs(data.deposit);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/api/deposit/update/${currentId}`,{
          ...inputs
        }
      );
      if (data) {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const { data } = await axios.delete(
        `${import.meta.env.VITE_BASE_URL}/api/deposit/delete/${currentId}`
      );
      if (data) {
    ...
    <Layout>
      <div className="w-full">
        <div className="text-center py-4 text-xl font-semibold">
          <h2>Bank Transaction - Deposit</h2>
        </div>
                        ...
                        ? deposit?.map((item) => (
                            <>
                              ...
                              >
                                ...
                                <td className="px-5 py-5 text-lg flex gap-3 items-center border-b border-gray-200">
                                  <button
                                    title="Edit"
                                    className="text-teal-600 text-xl"
                                    onClick={() => fetchSingleDeposit(item._id)}
                                  >
                                    <FaEdit />
                                  </button>
                                </td>
